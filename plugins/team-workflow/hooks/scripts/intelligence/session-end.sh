...
Co-Authored-By: ia-tools session-end hook <noreply@ia-tools>" \
  ...
  # `gh pr create` infers the repo from cwd; run it inside $repo so it lands
  # on the right one (was using `gh repo view` from the hook's cwd, which
  # broke multi-repo flows).
  (
    cd "$repo" || exit 1
    gh pr create \
      --base "$main_branch" \
...
- <concrete behavioral change: verify X before Y, allocate more turns for Z, add W to CLAUDE.md>"
    ...
    if [ -n "$lead_result" ]; then
      ...
      # Fallback when extraction returns empty: keep a minimal record so the
      # feature is not lost from the memory timeline.
      {
        printf '\n## %s — %s  [%s]\n' "$date_now" "$feature" "${stacks:-unknown}"
        printf '> auto: extraction empty — basic metadata\n'
        printf '- PRs: %s\n' "${pr_urls:-none}"
...
parse_worktrees() {
  awk '
    ...
    /^[[:space:]]*-?[[:space:]]*repo:[[:space:]]/ && repo == "" { gsub(/^[[:space:]]*-?[[:space:]]*repo:[[:space:]]*/, ""); repo = $0 }
