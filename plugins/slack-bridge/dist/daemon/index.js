...
import { resolveChannel, resolveUser, startListener } from './listener.js';
...
// ─── Slack listener ─────────────────────────────────────────────────
const app = await startListener({ botToken, appToken }, async (event) => {
    ...
    log(`[route] #${channelName} ${userName}: "${event.text.slice(0, 60)}" → ${targets.length} subscriber(s)`);
