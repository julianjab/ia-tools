    ...
    const server = createServer(async (req, res) => {
        ...
        if (req.method === 'POST' && path === '/subscribe') {
            try {
                ...
                log(`[api] +subscriber :${body.port} (${body.label ?? '-'}) filters=${JSON.stringify(sub.filters)} regexp=${JSON.stringify(body.regexp ?? {})}`);
                json(res, 200, sub);
            }
            catch (err) {
        ...
        if (req.method === 'POST' && path.startsWith('/claim/')) {
            ...
            try {
                ...
                if (existing !== undefined) {
                    const resp = { claimed: false, claimed_by: existing };
                    log(`[claim] ${messageTs} already claimed by :${existing}, rejected :${body.subscriber_port}`);
                    json(res, 409, resp);
                    return;
                }
