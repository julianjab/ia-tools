/**
 * session-manager role prompt.
 *
 * Originally lived at plugins/team-workflow/agents/session-manager.md and was
 * injected by the team-workflow SessionStart hook. The hook is gone — the role
 * is now embedded in the slack-bridge MCP `instructions` field, so it only
 * loads when slack-bridge is actually mounted (i.e. when Claude is started
 * with --dangerously-load-development-channels and the slack-bridge plugin).
 *
 * Sub-sessions (IA_TOOLS_ROLE=orchestrator) skip this prompt — start-session.sh
 * injects orchestrator.md natively via --append-system-prompt-file.
 */
export const SESSION_MANAGER_PROMPT = `# session-manager — Main Session Router
