...
## S19. Prohibition prose about actions outside the skill's scope

- **Signal**: Skill body contains lines like `Do NOT <verb>`, `Never <verb>`, `Don't <verb>`, `Avoid <verb>ing`, `must not <verb>`, `you should not <verb>` where the prohibited action is something the skill no longer does (because its `allowed-tools` were narrowed, because the work moved to another skill, etc.).
- **Why**: A prohibition forces the model to load the prohibited concept into its working context just to exclude it — every "Do NOT call the daemon directly" line is one more token of distraction from what the skill actually orchestrates. The standard for prompt prose is to **describe the positive action** ("Invoke `slack-bridge:reply` to publish replies") and **omit prohibitions about non-actions**. Legitimate hard constraints that ARE in-scope (e.g. "never force-push without confirmation" when the skill has unrestricted `Bash`) are fine — those are real invariants the model must enforce.
- **Fix**: For each negation, ask: does the skill's `allowed-tools` (or the body's documented capabilities) actually let it do the prohibited thing? If no, **delete the line entirely** and rely on the positive description elsewhere in the body. If yes (a real constraint), rephrase the line to state what the skill DOES instead: "Push via `--force-with-lease` only" instead of "Do NOT push with --force".
- **Heuristic for `/audit-skill`**: scan the body for the case-insensitive ERE `(^|[^a-zA-Z])(Do NOT|Never|Don't|Avoid)[[:space:]]+[a-z]`. Flag each match as MEDIUM with the suggestion to rewrite positively or delete. Headings, fenced code blocks, and lines that contain `invariant`, `constraint`, or the literal uppercase word `MUST` are exempt. The regex omits `cannot` / `must not` / `should not` — those forms are too often descriptive in real prose and produce noisy false positives.
